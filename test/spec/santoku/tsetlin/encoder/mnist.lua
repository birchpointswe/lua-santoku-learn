...
local SFFS_TYPE = "graph"
local SFFS_METHOD = "prefix"
local SFFS_DIRECTION = "forward"
local SFFS_FLOATING = true
...
test("tsetlin", function ()
  ...
  if RANKS then
    ...
    graph_ranks:fill(0, 0, dataset.n_visible)
    graph_ranks:fill(1, dataset.n_visible, dataset.n_visible + 10)
    local graph_supervision = ivec.create()
    graph_supervision:copy(train.ids)
    graph_supervision:lookup(dataset.solutions)
    graph_supervision:add_scaled(10)
