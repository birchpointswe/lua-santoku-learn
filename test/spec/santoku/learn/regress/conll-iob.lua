...
local cfg = {
  ...
  tag = { kernel = { "expcos" },
    ...
    search_trials = 0 },
  type = { kernel = { "cosine", "expcos", "geolaplace", "matern52", "rq", "arccos1" },
    lambda = { min = 1e-4, max = 1e1, log = true },
    propensity_a = { min = 0, max = 8 }, propensity_b = { min = 0, max = 16 },
    search_trials = 100 },
  vit = { min = 0, max = 1.5, tol = 0.01 },
  stack = { k = 5 },
  ctx = { iob = true },
}

local N_TYPES = 4
local NL = 2 * N_TYPES + 1
local O_LABEL = 2 * N_TYPES
local CTX_NT = cfg.ctx.iob and NL or N_TYPES
...
test("conll-iob", function ()
  ...
  do
    ...
    raw_ts_o, raw_ts_x, raw_ts_v = tso, tsx, fvec.create(tsv)
  ...
  local tr_oof_iob = ivec.create(tr_n); tr_oof_iob:zero()
  do
    local K = cfg.stack.k
