...
test("tokenizer", function ()
  ...
  test("normalize: lowercases, folds accents, collapses whitespace", function ()
    local out = tokenizer.normalize({ "Caf\195\169  AU\tLait ", "", "  x  " })
    assert(#out == 3)
    assert(out[1] == "cafe au lait")
    assert(out[2] == "")
    assert(out[3] == "x")
  end)

  test("normalize then extract feeds mode=words with folded tokens", function ()
    local raw = { "Caf\195\169 Noir", "cafe noir" }
    local norm = tokenizer.normalize(raw)
    local woff, ws, we = tokenizer.extract({ n = #norm, texts = norm, pattern = word_prog })
    local T = spans.create({ offsets = woff, s = ws, e = we })
    local w = tokenizer.create({ ngram_min = 1, ngram_max = 2, mode = "words" })
    local X = w:fit({ texts = norm, tokens = T })
    assert(X:rows(ivec.create({ 0 })):eq(X:rows(ivec.create({ 1 }))))
  end)
