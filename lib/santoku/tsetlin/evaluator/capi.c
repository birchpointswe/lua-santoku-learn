...
static inline int tm_optimize_clustering (lua_State *L)
{
  ...
  for (uint64_t m = min_margin; m <= max_margin; m ++) {
    tk_cluster_dsu(L, hbi, m, &ids, &assignments, &n_clusters);
    ...
    if (result.f1 > best.f1) {
      ...
      i_assign = luaL_ref(L, LUA_REGISTRYINDEX);
      i_ids = luaL_ref(L, LUA_REGISTRYINDEX);
  ...
  lua_rawgeti(L, LUA_REGISTRYINDEX, i_ids);
  lua_rawgeti(L, LUA_REGISTRYINDEX, i_assign);
  luaL_unref(L, LUA_REGISTRYINDEX, i_ids);
  luaL_unref(L, LUA_REGISTRYINDEX, i_assign);
  lua_pushinteger(L, (int64_t) best_n);
...
static inline int tm_optimize_retrieval (lua_State *L)
{
  ...
  state.mask = NULL;
  state.codes = codes;
  state.id_code = ids == NULL ? NULL : tk_iumap_from_ivec(ids);
  state.pos = pos;
  state.neg = neg;
  state.n_pos = n_pos;
  state.n_neg = n_neg;
