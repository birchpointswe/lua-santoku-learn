...
static inline void tk_cluster_dsu (
  ...
  tk_pvec_t *ptmp = (hbi || ann) ? tk_pvec_create(L, 0, 0, 0) : NULL;
  tk_rvec_t *rtmp = (!ptmp && inv) ? tk_rvec_create(L, 0, 0, 0) : NULL;

  tk_ivec_t *ids = (*idsp) =
    hbi ? tk_iumap_keys(L, hbi->uid_sid) :
    ann ? tk_iumap_keys(L, ann->uid_sid) :
    inv ? tk_iumap_keys(L, inv->uid_sid) : tk_ivec_create(L, 0, 0, 0);
  ...
  tk_ivec_t *assignments = (*assignmentsp) = tk_ivec_create(L, ids->n, 0, 0);
  int kha;
  khint_t khi;
  int64_t idx;
