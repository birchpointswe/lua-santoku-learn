...
static inline void tm_add_transatives (
  ...
  for (int64_t u = 0; u < (int64_t) graph->n_nodes; u ++) {
    ...
    for (uint64_t hop = 1; hop < n_hops; hop ++) {
      ...
      roaring64_bitmap_clear(frontier);
      roaring64_bitmap_or_inplace(frontier, next);
    }
  ...
  for (int64_t u = 0; u < (int64_t) graph->n_nodes; u ++) {
    ...
    ks_introsort(candidates_desc, candidates.n, candidates.a);
...
static tk_graph_t *tm_graph_create (
  ...
  tk_graph_t *graph = (tk_graph_t *) lua_newuserdata(L, sizeof(tk_graph_t));
  luaL_getmetatable(L, TK_MT_GRAPH);
...
int luaopen_santoku_tsetlin_graph (lua_State *L)
{
  ...
  luaL_newmetatable(L, TK_MT_GRAPH);
  lua_pushcfunction(L, tm_graph_gc);
