...
static inline int tk_corex_train (lua_State *L) {
  ...
  _tk_corex_train(L, C, set_bits, n_set_bits, n_samples, max_iter, i_each);
  return 0;
}
...
static inline int tk_corex_load (lua_State *L)
{
  ...
  tk_corex_t *C = (tk_corex_t *)
    lua_newuserdata(L, sizeof(tk_corex_t));
  memset(C, 0, sizeof(tk_corex_t));
  luaL_getmetatable(L, MT_COREX);
  lua_setmetatable(L, -2);
  lua_newtable(L);
  lua_pushvalue(L, -2);
  tk_lua_register(L, mt_fns, 1);
  lua_remove(L, -2);
...
static inline int tk_corex_create (lua_State *L)
{
  ...
  tk_corex_t *C = (tk_corex_t *) lua_newuserdata(L, sizeof(tk_corex_t));
  luaL_getmetatable(L, MT_COREX);
  lua_setmetatable(L, -2);
  tk_corex_init(L, C, lam, spa, tmin, ttc, anchor, tile_sblock, tile_vblock, n_visible, n_hidden, n_threads);
  lua_newtable(L);
  lua_pushvalue(L, -2);
  tk_lua_register(L, mt_fns, 1);
  return 1;
...
int luaopen_santoku_corex (lua_State *L)
{
  ...
  luaL_newmetatable(L, MT_COREX);
  lua_pushcfunction(L, tk_corex_gc);
